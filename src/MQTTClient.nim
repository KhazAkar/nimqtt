...
## !!!Ignored construct:  # ! defined ( ) [NewLine] # [NewLine] # defined ( __cplusplus ) [NewLine] extern C { # [NewLine] # < stdio . h > [NewLine]
## /// @endcond
##  # MQTTExportDeclarations.h [NewLine] # MQTTProperties.h [NewLine] # MQTTReasonCodes.h [NewLine] # MQTTSubscribeOpts.h [NewLine] # ! defined ( NO_PERSISTENCE ) [NewLine] # MQTTClientPersistence.h [NewLine] # [NewLine]
##  Return code: No error. Indicates successful completion of an MQTT client
##  operation.
##  # MQTTCLIENT_SUCCESS 0 [NewLine]
##  Return code: A generic error code indicating the failure of an MQTT client
##  operation.
##  # MQTTCLIENT_FAILURE - 1 [NewLine]  error code -2 is MQTTCLIENT_PERSISTENCE_ERROR
##  Return code: The client is disconnected.
##  # MQTTCLIENT_DISCONNECTED - 3 [NewLine]
##  Return code: The maximum number of messages allowed to be simultaneously
##  in-flight has been reached.
##  # MQTTCLIENT_MAX_MESSAGES_INFLIGHT - 4 [NewLine]
##  Return code: An invalid UTF-8 string has been detected.
##  # MQTTCLIENT_BAD_UTF8_STRING - 5 [NewLine]
##  Return code: A NULL parameter has been supplied when this is invalid.
##  # MQTTCLIENT_NULL_PARAMETER - 6 [NewLine]
...
##  # MQTTCLIENT_BAD_PROTOCOL - 14 [NewLine]
##  Return code: option not applicable to the requested version of MQTT
##  # MQTTCLIENT_BAD_MQTT_OPTION - 15 [NewLine]
##  Return code: call not applicable to the requested version of MQTT
##  # MQTTCLIENT_WRONG_MQTT_VERSION - 16 [NewLine]
##  Return code: 0 length will topic on connect
##  # MQTTCLIENT_0_LEN_WILL_TOPIC - 17 [NewLine]
##  Default MQTT version to connect with.  Use 3.1.1 then fall back to 3.1
##  # MQTTVERSION_DEFAULT 0 [NewLine]
##  MQTT version to connect with: 3.1
##  # MQTTVERSION_3_1 3 [NewLine]
##  MQTT version to connect with: 3.1.1
##  # MQTTVERSION_3_1_1 4 [NewLine]
##  MQTT version to connect with: 5
##  # MQTTVERSION_5 5 [NewLine]
##  Bad return code from subscribe, as defined in the 3.1.1 specification
##  # MQTT_BAD_SUBSCRIBE 0x80 [NewLine]
##   Initialization options
##  typedef struct {  The eyecatcher for this structure.  Must be MQTG. char struct_id [ 4 ] ;  The version number of this structure.  Must be 0 int struct_version ;  1 = we do openssl init, 0 = leave it to the application int do_openssl_init ; } MQTTClient_init_options ; # MQTTClient_init_options_initializer { { [char literal] , [char literal] , [char literal] , [char literal] } , 0 , 0 } [NewLine]
...
##  typedef int MQTTClient_deliveryToken ; typedef int MQTTClient_token ;
...
##  typedef struct {  The eyecatcher for this structure.  must be MQTM. char struct_id [ 4 ] ;  The version number of this structure.  Must be 0 or 1
##   0 indicates no message properties int struct_version ;  The length of the MQTT message payload in bytes. int payloadlen ;  A pointer to the payload of the MQTT message. void * payload ;
...
##  duplicate message.
##  int dup ;  The message identifier is normally reserved for internal use by the
##  MQTT client and server.
##  int msgid ;
##  The MQTT V5 properties associated with the message.
##  MQTTProperties properties ; } MQTTClient_message ; # MQTTClient_message_initializer { { [char literal] , [char literal] , [char literal] , [char literal] } , 1 , 0 , NULL , 0 , 0 , 0 , 0 , MQTTProperties_initializer } [NewLine]
...
##  typedef void MQTTClient_published ( void * context , int dt , int packet_type , MQTTProperties * properties , enum MQTTReasonCodes reasonCode ) ; LIBMQTT_API int MQTTClient_setPublished ( MQTTClient handle , void * context , MQTTClient_published * co ) ;
...
##  LIBMQTT_API int MQTTClient_create ( MQTTClient * handle , const char * serverURI , const char * clientId , int persistence_type , void * persistence_context ) ;  Options for the ::MQTTClient_createWithOptions call typedef struct {  The eyecatcher for this structure.  must be MQCO. char struct_id [ 4 ] ;  The version number of this structure.  Must be 0 int struct_version ;  Whether the MQTT version is 3.1, 3.1.1, or 5.  To use V5, this must be set.
...
##   3.1.1 and attempting to read 5.0 persisted messages will result in an error on create. int MQTTVersion ; } MQTTClient_createOptions ; # MQTTClient_createOptions_initializer { { [char literal] , [char literal] , [char literal] , [char literal] } , 0 , MQTTVERSION_DEFAULT } [NewLine]
...
##  required.
##  typedef struct {  The eyecatcher for this structure.  must be MQTW. char struct_id [ 4 ] ;  The version number of this structure.  Must be 0 or 1
## 		   0 means there is no binary payload option
##  int struct_version ;  The LWT topic to which the LWT message will be published. const char * topicName ;  The LWT payload in string form. const char * message ;
##  The retained flag for the LWT message (see MQTTClient_message.retained).
##  int retained ;
##  The quality of service setting for the LWT message (see
##  MQTTClient_message.qos and @ref qos).
##  int qos ;  The LWT payload in binary form. This is only checked and used if the message option is NULL struct { int len ; < binary payload length const void * data ; < binary payload data } payload ; } MQTTClient_willOptions ; # MQTTClient_willOptions_initializer { { [char literal] , [char literal] , [char literal] , [char literal] } , 1 , NULL , NULL , 0 , 0 , { 0 , NULL } } [NewLine] # MQTT_SSL_VERSION_DEFAULT 0 [NewLine] # MQTT_SSL_VERSION_TLS_1_0 1 [NewLine] # MQTT_SSL_VERSION_TLS_1_1 2 [NewLine] # MQTT_SSL_VERSION_TLS_1_2 3 [NewLine]
...
##  typedef struct {  The eyecatcher for this structure.  Must be MQTS char struct_id [ 4 ] ;  The version number of this structure. Must be 0, 1, 2, 3, 4 or 5.
...
##  int struct_version ;  The file in PEM format containing the public digital certificates trusted by the client. const char * trustStore ;  The file in PEM format containing the public certificate chain of the client. It may also include
##  the client's private key.
##  const char * keyStore ;  If not included in the sslKeyStore, this setting points to the file in PEM format containing
##  the client's private key.
##  const char * privateKey ;  The password to load the client's privateKey if encrypted. const char * privateKeyPassword ;
...
##  const char * enabledCipherSuites ;  True/False option to enable verification of the server certificate * int enableServerCertAuth ;  The SSL/TLS version to use. Specify one of MQTT_SSL_VERSION_DEFAULT (0),
...
##  Exists only if struct_version >= 5
##  unsigned int protos_len ; } MQTTClient_SSLOptions ; # MQTTClient_SSLOptions_initializer { { [char literal] , [char literal] , [char literal] , [char literal] } , 5 , NULL , NULL , NULL , NULL , NULL , 1 , MQTT_SSL_VERSION_DEFAULT , 0 , NULL , NULL , NULL , NULL , NULL , 0 , NULL , 0 } [NewLine]
...
##  typedef struct {  The eyecatcher for this structure.  must be MQTC. char struct_id [ 4 ] ;  The version number of this structure.  Must be 0, 1, 2, 3, 4, 5, 6, 7 or 8.
...
##  int struct_version ;  The "keep alive" interval, measured in seconds, defines the maximum time
...
##  struct { const char * serverURI ; < the serverURI connected to int MQTTVersion ; < the MQTT version used to connect with int sessionPresent ; < if the MQTT version is 3.1.1, the value of sessionPresent returned in the connack } returned ;
##  Optional binary password.  Only checked and used if the password option is NULL
##  struct { int len ; < binary password length const void * data ; < binary password data } binarypwd ;
...
##  HTTPS proxy
##  const char * httpsProxy ; } MQTTClient_connectOptions ;  Initializer for connect options for MQTT 3.1.1 non-WebSocket connections # MQTTClient_connectOptions_initializer { { [char literal] , [char literal] , [char literal] , [char literal] } , 8 , 60 , 1 , 1 , NULL , NULL , NULL , 30 , 0 , NULL , 0 , NULL , MQTTVERSION_DEFAULT , { NULL , 0 , 0 } , { 0 , NULL } , - 1 , 0 , NULL , NULL , NULL } [NewLine]  Initializer for connect options for MQTT 5.0 non-WebSocket connections # MQTTClient_connectOptions_initializer5 { { [char literal] , [char literal] , [char literal] , [char literal] } , 8 , 60 , 0 , 1 , NULL , NULL , NULL , 30 , 0 , NULL , 0 , NULL , MQTTVERSION_5 , { NULL , 0 , 0 } , { 0 , NULL } , - 1 , 1 , NULL , NULL , NULL } [NewLine]  Initializer for connect options for MQTT 3.1.1 WebSockets connections.
##  The keepalive interval is set to 45 seconds to avoid webserver 60 second inactivity timeouts.
##  # MQTTClient_connectOptions_initializer_ws { { [char literal] , [char literal] , [char literal] , [char literal] } , 8 , 45 , 1 , 1 , NULL , NULL , NULL , 30 , 0 , NULL , 0 , NULL , MQTTVERSION_DEFAULT , { NULL , 0 , 0 } , { 0 , NULL } , - 1 , 0 , NULL , NULL , NULL } [NewLine]  Initializer for connect options for MQTT 5.0 WebSockets connections.
##  The keepalive interval is set to 45 seconds to avoid webserver 60 second inactivity timeouts.
##  # MQTTClient_connectOptions_initializer5_ws { { [char literal] , [char literal] , [char literal] , [char literal] } , 8 , 45 , 0 , 1 , NULL , NULL , NULL , 30 , 0 , NULL , 0 , NULL , MQTTVERSION_5 , { NULL , 0 , 0 } , { 0 , NULL } , - 1 , 1 , NULL , NULL , NULL } [NewLine]
...
##  from the server
##  LIBMQTT_API int MQTTClient_disconnect ( MQTTClient handle , int timeout ) ; LIBMQTT_API int MQTTClient_disconnect5 ( MQTTClient handle , int timeout , enum MQTTReasonCodes reason , MQTTProperties * props ) ;
##  This function allows the client application to test whether or not a
##  client is currently connected to the MQTT server.
##  @param handle A valid client handle from a successful call to
##  MQTTClient_create().
##  @return Boolean true if the client is connected, otherwise false.
##  LIBMQTT_API int MQTTClient_isConnected ( MQTTClient handle ) ;  Subscribe is synchronous.  QoS list parameter is changed on return to granted QoSs.
...
##  LIBMQTT_API void MQTTClient_destroy ( MQTTClient * handle ) ; enum MQTTCLIENT_TRACE_LEVELS { MQTTCLIENT_TRACE_MAXIMUM = 1 , MQTTCLIENT_TRACE_MEDIUM , MQTTCLIENT_TRACE_MINIMUM , MQTTCLIENT_TRACE_PROTOCOL , MQTTCLIENT_TRACE_ERROR , MQTTCLIENT_TRACE_SEVERE , MQTTCLIENT_TRACE_FATAL , } ;
...
##  LIBMQTT_API const char * MQTTClient_strerror ( int code ) ; # defined ( __cplusplus ) [NewLine] } # [NewLine] # [NewLine] !
##  @cond MQTTClient_main
